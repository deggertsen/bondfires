...
export const setStoreAccountTokenIfMissing = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!user || user.accountDeletionStatus) throw new Error('Account unavailable')
...
export const findSubscriptionForStoreSubject = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return (
      subscriptions.find(
        (subscription) =>
          subscription.platform === args.platform &&
...
export const purgeOldEvents = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    if (rows.length === 100)
      await ctx.scheduler.runAfter(0, internal.storeBilling.purgeOldEvents, {})
