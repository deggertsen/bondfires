...
/**
 * List users who have participated in camps the current user manages (is owner
 * or moderator of). Deduped by user ID and ordered by most recent interaction
 * first. Excludes the current user themselves.
 */
export const listInvitableCampContacts = query({
  args: {},
  handler: async (ctx) => {
    const userId = await auth.getUserId(ctx)
    if (!userId) return []
    ...
    // Get camps where user is owner or moderator
    const memberships = await ctx.db
      .query('campMembers')
    ...
    // Get all members across those camps
    const allMembers: { userId: string; joinedAt: number }[] = []
    for (const campId of campIds) {
      ...
      for (const m of members) {
        allMembers.push({ userId: m.userId, joinedAt: m._creationTime })
      }
    }

    // Dedupe by userId, exclude current user
    const unique = new Map<string, number>()
    for (const m of allMembers) {
      if (m.userId === userId) continue
      const existing = unique.get(m.userId)
      if (!existing || m.joinedAt > existing) {
        unique.set(m.userId, m.joinedAt)
      }
    }
