...
- `react-native-compressor` - Video metadata extraction
...
Video uploads use Bunny.net Stream through Convex actions. Keep Bunny credentials server-side in Convex environment variables:
...
New bondfire videos upload directly to Bunny.net Stream using TUS credentials generated by Convex. If uploads fail:

- Ensure you have a development build (not Expo Go)
- Check that Bunny Stream Convex environment variables are configured
