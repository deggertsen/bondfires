...
Video uploads use Mux direct uploads through Convex actions. Keep Mux credentials server-side in Convex environment variables:
...
New bondfire videos upload directly to Mux using direct upload URLs generated by Convex. If uploads fail:

- Ensure you have a development build (not Expo Go)
- Check that Mux Convex environment variables are configured
