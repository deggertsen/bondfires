...
export function LiveRecordScreen({
  ...
  // Memory pressure listener — RTMP encoding + camera = heavy memory load.
  // On iOS, the native module emits a 'memory_warning' error event that the
  // useLivePublisher hook catches and logs as 'live:memory_warning'.
  // On Android, we listen for the native 'bondfireMemoryWarning' DeviceEvent.
  useEffect(() => {
    if (phase !== 'recording' && liveStatus !== 'connecting') return
    if (Platform.OS !== 'android') return

    const handler = (payload: { level?: string }) => {
    ...
    const sub = DeviceEventEmitter.addListener('bondfireMemoryWarning', handler)
  ...
  useEffect(() => {
    ...
    const checkThermal = async () => {
      try {
        // Use the live publisher's native module for thermal state
