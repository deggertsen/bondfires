...
export default function CampInviteScreen() {
  ...
  if (!isAuthenticated && code) {
    return <Redirect href={routes.loginWithCampInvite(code)} />
  }
  ...
  const handleRedeem = useCallback(async () => {
    ...
    try {
      ...
      setStatus('success')
      setTimeout(() => {
        router.replace(routes.camp(result.campId))
      }, 1500)
    } catch (err) {
      const message =
      ...
      <YStack flex={1} alignItems="center" justifyContent="center" paddingHorizontal={32} gap={20}>
        ...
        {status === 'error' && (
          <>
            ...
            <Text fontSize={15} color={bondfireColors.ash} textAlign="center" lineHeight={22}>
              {error ?? 'Something went wrong. The invite may have expired or the camp is full.'}
            </Text>
            <Button
              variant="primary"
              marginTop={8}
