...
export default function CampsScreen() {
  ...
  const archivedCamps = useMemo(() => {
    if (!myCamps) return []
    return myCamps.filter((camp) => camp.status === 'archived')
  }, [myCamps])
  ...
  const listItems = useMemo<CampListItem[]>(() => {
    ...
    // Add archived section header when not searching and there are archived camps
        ...
        ListFooterComponent={
          query.trim().length > 0 || archivedCamps.length === 0 ? null : (
            <YStack paddingBottom={32}>
              ...
              {archivedExpanded
                ? archivedCamps.map((camp) => (
                    <Pressable
                      key={camp._id}
                      onPress={() => router.push(`/(main)/camp/${camp._id}` as RelativePathString)}
                    >
                      <XStack
                        paddingHorizontal={16}
                        paddingVertical={12}
