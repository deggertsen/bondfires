 ...
 * EXPO_PUBLIC_LOCAL_BACKUP_RECORDING=1 at build time to enable. Expo only
 * inlines statically referenced EXPO_PUBLIC variables, so keep this as a
 * direct process.env property access (no computed lookup or cast wrapper).
 */
export function isLocalBackupFlagEnabled(): boolean {
  return process.env.EXPO_PUBLIC_LOCAL_BACKUP_RECORDING === '1'
}
