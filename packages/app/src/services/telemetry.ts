...
type MutationApiType = ReturnType<typeof import('convex/react').useMutation>
...
interface TelemetryLoggerOptions {
  /** Convex mutation reference for single-entry creation. */
  mutationCreate?: unknown
  /** Convex mutation reference for batch creation. */
  mutationCreateBatch?: unknown
}
...
export class TelemetryLogger {
  ...
  private _mutationCreate: ((args: unknown) => Promise<unknown>) | null = null
  ...
  /**
   * Initialize the logger with a Convex mutation ref and start capturing.
  ...
  init(api: {
    ...
    this._mutationCreate = api.create
  ...
  private async sendBatch(): Promise<void> {
    ...
    try {
      ...
      // Silently drop — we don't want telemetry failures to surface
  ...
  private installConsoleOverrides(): void {
    console.error = (...args: unknown[]) => {
      ...
      // Don't double-log — telemetry.error() already calls _origConsoleError
      // We only capture direct calls that bypass our API
    }

    console.warn = (...args: unknown[]) => {
      this._origConsoleWarn(...args)
      // Same — telemetry.warn() already echoes to original console
    }
  }
}
