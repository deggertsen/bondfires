...
export async function startBackgroundUpload(
  ...
  if (autoStart) {
    ...
    processUploadTask(taskId, options).catch((error) => {
      telemetry.error('upload:task', 'Background upload task failed', { taskId, error: String(error) })
...
async function processUploadTask(taskId: string, options: BackgroundUploadOptions): Promise<void> {
  ...
  try {
    ...
    telemetry.error('upload:error', 'Upload error', { taskId, attempt: attemptCount, error: normalizedError.message })
    attemptCount++
    ...
    if (attemptCount < MAX_RETRIES) {
      ...
      setTimeout(() => {
        processUploadTask(taskId, options).catch((err) => {
          telemetry.error('upload:retry', 'Retry failed', { taskId, attempt: attemptCount, error: String(err) })
        })
      }, retryDelay)
    } else {
      // Max retries reached
...
export async function resumePendingUploads(
  ...
  for (const task of pendingTasks) {
    ...
    processUploadTask(task.id, taskOptions).catch((error) => {
      telemetry.error('upload:resume', 'Failed to resume task', { taskId: task.id, error: String(error) })
    })
  }
}
