...
Functions:
- `addReaction(bondfireId?, bondfireVideoId?, emoji, timestampMs)` — mutation. Validates exactly one video reference. Denormalizes userDisplayName + userPhotoUrl from users table. Returns created reaction or null on failure. Wrap in `withUserFacingActionErrors` from `convex/errors.ts`.
...
**New files:**
- `packages/app/src/hooks/usePresence.ts` — Hook managing heartbeat interval (30s), leaveViewing on unmount/blur, reactive `useQuery(api.presence.listViewers)` subscription, client-side self-exclusion
