...
| Camps public discovery | `camps.list`, `camps.listPage`, `camps.get` | Deliberate signed-out discovery; camp lifecycle/rule visibility filters apply. |
...
| Upload completion | `videos.getUploadCompletion`, `videos.monitorUploadCompletion` | Authenticated upload owner; bounded upload ID. Reads only the owned upload's status and transactionally deduplicates bounded server recovery checks. |
