...
Last reviewed: 2026-08-31. This is an inventory of every function exported with
...
| Public configuration writes | `publicConfig.setMinVersion` | Admin-only correction and UI are owned by PR #215. This branch must rebase after it and preserve that authorization. |
...
| User self-service | `users.recordActive`, `users.current`, `users.updateProfile`, `users.setThemePreference`, `users.generateProfilePhotoUploadUrl`, `users.updateProfilePhoto`, `users.deleteAccount`, `users.isAdmin` | Authenticated self; server derives user ID. Account deletion is superseded by PR #217. |
...
| Camp maintenance | `camps.seedLaunchCamps`, `camps.resetAndReseed`, `camps.adminBackfill`, `bondfires.cleanupExpiredPrivateCampVideos` | Admin only. Empty-table seeding no longer bypasses admin. Destructive CLI variants remain internal functions. |
...
| Reactions and presence | `videoReactions.addReaction`, `videoReactions.getReactions`, `videoReactions.getRecentEmojis`, `presence.heartbeat`, `presence.leaveViewing`, `presence.listViewers` | Authenticated writes and visible reads; PR #220 adds the shared block/visibility and reaction/presence abuse policy. This PR must compose after its rebase without duplicating that feature. |
| Reports | `reports.submit`, `reports.hasReportedBondfire`, `reports.hasReportedBondfireVideo` | Authenticated reporter and target checks; PR #220 adds moderation/block integration and report throttling. |
...
| Client telemetry | `clientLogs.create`, `clientLogs.createBatch`, `clientLogs.list`, `clientLogs.summary` | Writes are authenticated/self-attributed; admin reads. PR #216 owns server timestamps, payload bounds, redaction, and rate limiting. |
...
| Video playback | `videos.getVideoUrls`, `videos.getVideoUrlsBatch`, `videos.getThumbnailUrl`, `videos.getThumbnailUrlsBatch` | Public playback IDs may resolve public URLs; signed playback requires shared object visibility. Batch sizes are capped. |
...
- Merge PRs #215 and #216 first, update and merge #217, then rebase #218. PR
  #220 must rebase on all of those before this branch rebases on #220. Preserve
  #218's admin-only launch seeding and #220's block-aware visibility and
  reaction/presence/report rate controls. During the final rebase, move this
  branch's `abuseRateLimits` cleanup into #217's bounded deletion worker.
